...
class Card extends \App\Common\Models\Weixincard\Card
{
     ...
     *
     * @param string $card_id            
     * @param int $received_num            
     * @throws \Exception
     * @return boolean
     */
    public function incReceivedNum($card_id, $received_num = 1)
    {
        $query = array(
            'card_id' => $card_id
        );
     ...
     *
     * @param string $card_id            
     * @param int $give_by_friend_num            
     * @throws \Exception
     * @return boolean
     */
    public function incGiveByFriendNum($card_id, $give_by_friend_num = 1)
    {
        $query = array(
            'card_id' => $card_id
        );
     ...
     * @return boolean
     */
    public function incConsumedNum($card_id, $consumed_num = 1)
    {
        $query = array(
            'card_id' => $card_id
        );
     ...
     *
     * @param string $card_id            
     * @param int $deleted_num            
     * @throws \Exception
     * @return boolean
     */
    public function incDeletedNum($card_id, $deleted_num = 1)
    {
        $query = array(
            'card_id' => $card_id
        );
     ...
     *
     * @param string $card_id            
     * @param int $unavailable_num            
     * @throws \Exception
     * @return boolean
     */
    public function incUnavailableNum($card_id, $unavailable_num = 1)
    {
        $query = array(
            'card_id' => $card_id
        );
    ...
    public function getAndUpdateCardInfo($card_id)
    {
        ...
        $weixinCardInfo = $ret['card'];
        $rst = $this->record($weixinCardInfo);
        return $rst;
    }
     ...
     * 记录数据
     *
     * @param array $weixinCardInfo
     *            微信返回的信息
     * @param array $memo            
     * @throws \Exception
     * @return array
     */
    public function record(array $weixinCardInfo, array $memo = array())
    {
        $memo['weixinCardInfo'] = $weixinCardInfo;
        $card_type = strtoupper($weixinCardInfo['card_type']);
        $ret = array();
        switch ($card_type) {
            case 'GENERAL_COUPON':
                // 通用券
                $base_info = $weixinCardInfo['general_coupon']['base_info'];
                $objBaseInfo = $this->getBaseInfo($base_info);
                $default_detail = isset($weixinCardInfo['general_coupon']['default_detail']) ? trim($weixinCardInfo['general_coupon']['default_detail']) : "";
            ...
            case 'GROUPON':
                // 团购券
                $base_info = $weixinCardInfo['groupon']['base_info'];
                $objBaseInfo = $this->getBaseInfo($base_info);
                $deal_detail = isset($weixinCardInfo['groupon']['deal_detail']) ? trim($weixinCardInfo['groupon']['deal_detail']) : "";
            ...
            case 'DISCOUNT':
                // 折扣券
                $base_info = $weixinCardInfo['discount']['base_info'];
                $objBaseInfo = $this->getBaseInfo($base_info);
                $discount = isset($weixinCardInfo['discount']['discount']) ? intval($weixinCardInfo['discount']['discount']) : 0;
            ...
            case 'GIFT':
                // 礼品券
                $base_info = $weixinCardInfo['gift']['base_info'];
                $objBaseInfo = $this->getBaseInfo($base_info);
                $gift = isset($weixinCardInfo['gift']['gift']) ? trim($weixinCardInfo['gift']['gift']) : "";
            ...
            case 'CASH':
                // 代金券
                $base_info = $weixinCardInfo['cash']['base_info'];
                $objBaseInfo = $this->getBaseInfo($base_info);
                $least_cost = isset($weixinCardInfo['cash']['least_cost']) ? intval($weixinCardInfo['cash']['least_cost']) : 0;
                $reduce_cost = isset($weixinCardInfo['cash']['reduce_cost']) ? intval($weixinCardInfo['cash']['reduce_cost']) : 0;
            ...
            case 'MEMBER_CARD':
                // 会员卡
                $base_info = $weixinCardInfo['member_card']['base_info'];
                $objBaseInfo = $this->getBaseInfo($base_info);
                $supply_bonus = isset($weixinCardInfo['member_card']['supply_bonus']) ? trim($weixinCardInfo['member_card']['supply_bonus']) : false;
                $supply_balance = isset($weixinCardInfo['member_card']['supply_balance']) ? trim($weixinCardInfo['member_card']['supply_balance']) : false;
                $prerogative = isset($weixinCardInfo['member_card']['prerogative']) ? trim($weixinCardInfo['member_card']['prerogative']) : "";

                $bonus_cleared = isset($weixinCardInfo['member_card']['bonus_cleared']) ? trim($weixinCardInfo['member_card']['bonus_cleared']) : "";
                $bonus_rules = isset($weixinCardInfo['member_card']['bonus_rules']) ? trim($weixinCardInfo['member_card']['bonus_rules']) : "";
                $balance_rules = isset($weixinCardInfo['member_card']['balance_rules']) ? trim($weixinCardInfo['member_card']['balance_rules']) : "";
                $bind_old_card_url = isset($weixinCardInfo['member_card']['bind_old_card_url']) ? trim($weixinCardInfo['member_card']['bind_old_card_url']) : "";
                $activate_url = isset($weixinCardInfo['member_card']['activate_url']) ? trim($weixinCardInfo['member_card']['activate_url']) : "";

                $objCard = new MemberCard($objBaseInfo, $supply_bonus, $supply_balance, $prerogative);

                if (!empty($weixinCardInfo['member_card']['custom_field1']['name_type']) && !empty($weixinCardInfo['member_card']['custom_field1']['url'])) {
                    $custom_field1 = new CustomField($weixinCardInfo['member_card']['custom_field1']['name_type'], $weixinCardInfo['member_card']['custom_field1']['url']);
                    $objCard->set_custom_field1($custom_field1);
                }
                if (!empty($weixinCardInfo['member_card']['custom_field2']['name_type']) && !empty($weixinCardInfo['member_card']['custom_field2']['url'])) {
                    $custom_field2 = new CustomField($weixinCardInfo['member_card']['custom_field2']['name_type'], $weixinCardInfo['member_card']['custom_field2']['url']);
                    $objCard->set_custom_field2($custom_field2);
                }
                if (!empty($weixinCardInfo['member_card']['custom_field3']['name_type']) && !empty($weixinCardInfo['member_card']['custom_field3']['url'])) {
                    $custom_field3 = new CustomField($weixinCardInfo['member_card']['custom_field3']['name_type'], $weixinCardInfo['member_card']['custom_field3']['url']);
                    $objCard->set_custom_field3($custom_field3);
                }

                $need_push_on_view = empty($weixinCardInfo['member_card']['need_push_on_view']) ? NULL : trim($weixinCardInfo['member_card']['need_push_on_view']);
                if (!empty($weixinCardInfo['member_card']['custom_cell1']['name']) && !empty($weixinCardInfo['member_card']['custom_cell1']['url'])) {
                    $custom_cell1 = new CustomCell($weixinCardInfo['member_card']['custom_cell1']['name'], $weixinCardInfo['member_card']['custom_cell1']['url']);
                    if (!empty($weixinCardInfo['member_card']['custom_cell1']['tips'])) {
                        $custom_cell1->set_tips($weixinCardInfo['member_card']['custom_cell1']['tips']);
                    }
                    $objCard->set_custom_cell1($custom_cell1);
                }
                if (!empty($weixinCardInfo['member_card']['custom_cell2']['name']) && !empty($weixinCardInfo['member_card']['custom_cell2']['url'])) {
                    $custom_cell2 = new CustomCell($weixinCardInfo['member_card']['custom_cell2']['name'], $weixinCardInfo['member_card']['custom_cell2']['url']);
                    if (!empty($weixinCardInfo['member_card']['custom_cell2']['tips'])) {
                        $custom_cell2->set_tips($weixinCardInfo['member_card']['custom_cell2']['tips']);
                    }
            ...
            case 'SCENIC_TICKET':
                // 门票
                $base_info = $weixinCardInfo['scenic_ticket']['base_info'];
                $objBaseInfo = $this->getBaseInfo($base_info);
                $ticket_class = isset($weixinCardInfo['scenic_ticket']['ticket_class']) ? trim($weixinCardInfo['scenic_ticket']['ticket_class']) : "";
                $guide_url = isset($weixinCardInfo['scenic_ticket']['guide_url']) ? trim($weixinCardInfo['scenic_ticket']['guide_url']) : "";
            ...
            case 'MOVIE_TICKET':
                // 电影票
                $base_info = $weixinCardInfo['movie_ticket']['base_info'];
                $objBaseInfo = $this->getBaseInfo($base_info);
                $detail = isset($weixinCardInfo['movie_ticket']['detail']) ? trim($weixinCardInfo['movie_ticket']['detail']) : "";
            ...
            case 'BOARDING_PASS':
                // 飞机票
                $base_info = $weixinCardInfo['boarding_pass']['base_info'];
                $objBaseInfo = $this->getBaseInfo($base_info);
                $from = isset($weixinCardInfo['boarding_pass']['from']) ? trim($weixinCardInfo['boarding_pass']['from']) : "";
                $to = isset($weixinCardInfo['boarding_pass']['to']) ? trim($weixinCardInfo['boarding_pass']['to']) : "";
                $flight = isset($weixinCardInfo['boarding_pass']['flight']) ? trim($weixinCardInfo['boarding_pass']['flight']) : "";

                $departure_time = isset($weixinCardInfo['boarding_pass']['departure_time']) ? trim($weixinCardInfo['boarding_pass']['departure_time']) : "";
                $landing_time = isset($weixinCardInfo['boarding_pass']['landing_time']) ? trim($weixinCardInfo['boarding_pass']['landing_time']) : "";
                $check_in_url = isset($weixinCardInfo['boarding_pass']['check_in_url']) ? trim($weixinCardInfo['boarding_pass']['check_in_url']) : "";
                $gate = isset($weixinCardInfo['boarding_pass']['gate']) ? trim($weixinCardInfo['boarding_pass']['gate']) : "";
                $boarding_time = isset($weixinCardInfo['boarding_pass']['boarding_time']) ? trim($weixinCardInfo['boarding_pass']['boarding_time']) : "";
                $air_model = isset($weixinCardInfo['boarding_pass']['air_model']) ? trim($weixinCardInfo['boarding_pass']['air_model']) : "";
            ...
            case 'LUCKY_MONEY':
                // 红包
                $base_info = $weixinCardInfo['lucky_money']['base_info'];
                $objBaseInfo = $this->getBaseInfo($base_info);
     ...
     * @param array $memo            
     */
    public function recordBoardingPass(BoardingPass $objCard, array $memo = array())
    {
        ...
        $data[$card_type_low . '_' . 'from'] = (string) ($from);
        $data[$card_type_low . '_' . 'to'] = (string) ($to);
        $data[$card_type_low . '_' . 'flight'] = (string) ($flight);
        $data[$card_type_low . '_' . 'departure_time'] = (string) ($departure_time);
        $data[$card_type_low . '_' . 'landing_time'] = (string) ($landing_time);
        $data[$card_type_low . '_' . 'check_in_url'] = (string) ($check_in_url);
        $data[$card_type_low . '_' . 'gate'] = (string) ($gate);
        $data[$card_type_low . '_' . 'boarding_time'] = (string) ($boarding_time);
        $data[$card_type_low . '_' . 'air_model'] = (string) ($air_model);
     ...
     * @param array $memo            
     */
    public function recordCash(Cash $objCard, array $memo = array())
    {
        $card_id = $objCard->base_info->card_id;
        $card_type = $objCard->card_type;
        $base_info = $this->formatBaseInfoData($objCard);
        ...
        $card_type_low = strtolower($card_type);
        $data[$card_type_low . '_' . 'least_cost'] = intval($least_cost);
        $data[$card_type_low . '_' . 'reduce_cost'] = intval($reduce_cost);
     ...
     * @param array $memo            
     */
    public function recordDiscount(Discount $objCard, array $memo = array())
    {
        $card_id = $objCard->base_info->card_id;
        $card_type = $objCard->card_type;
        $base_info = $this->formatBaseInfoData($objCard);
        ...
        $card_type_low = strtolower($card_type);
        $data[$card_type_low . '_' . 'discount'] = intval($discount);
     ...
     * @param array $memo            
     */
    public function recordGeneralCoupon(GeneralCoupon $objCard, array $memo = array())
    {
        $card_id = $objCard->base_info->card_id;
        $card_type = $objCard->card_type;
        $base_info = $this->formatBaseInfoData($objCard);
        ...
        $card_type_low = strtolower($card_type);
        $data[$card_type_low . '_' . 'default_detail'] = (string) ($default_detail);
     ...
     * @param array $memo            
     */
    public function recordGift(Gift $objCard, array $memo = array())
    {
        $card_id = $objCard->base_info->card_id;
        $card_type = $objCard->card_type;
        $base_info = $this->formatBaseInfoData($objCard);
        ...
        $card_type_low = strtolower($card_type);
        $data[$card_type_low . '_' . 'gift'] = (string) ($gift);
     ...
     * @param array $memo            
     */
    public function recordGroupon(Groupon $objCard, array $memo = array())
    {
        $card_id = $objCard->base_info->card_id;
        $card_type = $objCard->card_type;
        $base_info = $this->formatBaseInfoData($objCard);
        ...
        $card_type_low = strtolower($card_type);
        $data[$card_type_low . '_' . 'deal_detail'] = (string) ($deal_detail);
     ...
     * @param array $memo            
     */
    public function recordLuckyMoney(LuckyMoney $objCard, array $memo = array())
    {
        $card_id = $objCard->base_info->card_id;
        $card_type = $objCard->card_type;
        $base_info = $this->formatBaseInfoData($objCard);
     ...
     * @param array $memo            
     */
    public function recordMemberCard(MemberCard $objCard, array $memo = array())
    {
        $card_id = $objCard->base_info->card_id;
        $card_type = $objCard->card_type;
        $base_info = $this->formatBaseInfoData($objCard);
        ...
        $data[$card_type_low . '_' . 'supply_bonus'] = $supply_bonus;
        $data[$card_type_low . '_' . 'supply_balance'] = $supply_balance;
        $data[$card_type_low . '_' . 'bonus_cleared'] = (string) ($bonus_cleared);
        $data[$card_type_low . '_' . 'bonus_rules'] = (string) ($bonus_rules);
        $data[$card_type_low . '_' . 'balance_rules'] = (string) ($balance_rules);
        $data[$card_type_low . '_' . 'prerogative'] = (string) ($prerogative);
        $data[$card_type_low . '_' . 'bind_old_card_url'] = (string) ($bind_old_card_url);
        $data[$card_type_low . '_' . 'activate_url'] = (string) ($activate_url);
        $data[$card_type_low . '_' . 'custom_field1'] = $custom_field1;
        $data[$card_type_low . '_' . 'custom_field2'] = $custom_field2;
        $data[$card_type_low . '_' . 'custom_field3'] = $custom_field3;
        $data[$card_type_low . '_' . 'custom_cell1'] = $custom_cell1;
        $data[$card_type_low . '_' . 'custom_cell2'] = $custom_cell2;
        $data[$card_type_low . '_' . 'need_push_on_view'] = $need_push_on_view;
     ...
     * @param array $memo            
     */
    public function recordMovieTicket(MovieTicket $objCard, array $memo = array())
    {
        $card_id = $objCard->base_info->card_id;
        $card_type = $objCard->card_type;
        $base_info = $this->formatBaseInfoData($objCard);
        ...
        $data[$card_type_low . '_' . 'detail'] = (string) ($detail);
     ...
     * @param array $memo            
     */
    public function recordScenicTicket(ScenicTicket $objCard, array $memo = array())
    {
        $card_id = $objCard->base_info->card_id;
        $card_type = $objCard->card_type;
        $base_info = $this->formatBaseInfoData($objCard);
        ...
        $data[$card_type_low . '_' . 'ticket_class'] = (string) ($ticket_class);
        $data[$card_type_low . '_' . 'guide_url'] = (string) ($guide_url);
    ...
    public function create(array $card, array $colors)
    {
        ...
        $location_id_list = empty($card['location_id_list']) ? NULL : explode(',', $card['location_id_list']);
     ...
     */
    protected function getBaseInfo(array $base_info)
    {
        ...
        $use_custom_code = empty($base_info['use_custom_code']) ? false : true;
        $bind_openid = empty($base_info['bind_openid']) ? false : true;
        $can_share = empty($base_info['can_share']) ? false : true;
        $can_give_friend = empty($base_info['can_give_friend']) ? false : true;

        $use_all_locations = isset($base_info['use_all_locations']) ? intval($base_info['use_all_locations']) : 0;

        if (empty($use_all_locations)) {
            $location_id_list = empty($base_info['location_id_list']) ? array() : $base_info['location_id_list'];
        } else {
            $location_id_list = array();
        }
        $date_info_type = trim($base_info['date_info']['type']);
        ...
        $can_shake = empty($base_info['can_shake']) ? false : true;
    ...
    protected function formatBaseInfoData(CardBase $objCard)
    {
        ...
        $use_custom_code = $objCard->base_info->use_custom_code ? 1 : 0;
        $bind_openid = $objCard->base_info->bind_openid ? 1 : 0;
        $can_share = $objCard->base_info->can_share ? 1 : 0;
        $can_give_friend = $objCard->base_info->can_give_friend ? 1 : 0;
        ...
        $can_shake = $objCard->base_info->can_shake ? 1 : 0;
        ...
        $base_info['use_custom_code'] = intval($use_custom_code);
        $base_info['bind_openid'] = intval($bind_openid);
        $base_info['can_share'] = intval($can_share);
        ...
        $base_info['date_info_begin_timestamp'] = \App\Common\Utils\Helper::getCurrentTime(intval($date_info_begin_timestamp));
        $base_info['date_info_end_timestamp'] = \App\Common\Utils\Helper::getCurrentTime(intval($date_info_end_timestamp));
        ...
        $base_info['can_shake'] = intval($can_shake);
