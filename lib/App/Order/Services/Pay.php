...
class Pay
{
    ...
    function __construct()
    {
        ...
        $this->modelPointsService = new \App\Points\Services\Api();
    }
                    ...
                    default:
                        ...
                        if (!empty($buyerInfo['inviter_id'])) {
                            ...
                            if (!empty($invitationInfo)) {
                                ...
                                $this->modelPointsService->addOrReduce(POINTS_CATEGORY1, $invitationInfo['user_id'], $invitationInfo['user_name'], $invitationInfo['user_headimgurl'], $orderPayInfo['_id'], $now, $pointsRuleInfo['points'], $pointsRuleInfo['item_category'], $pointsRuleInfo['item']);
                                $pointsRuleInfo = $this->modelPointsRule->getInfoByCategoryAndCode(POINTS_CATEGORY2, 'invitation');
                                $this->modelPointsService->addOrReduce(POINTS_CATEGORY2, $invitationInfo['user_id'], $invitationInfo['user_name'], $invitationInfo['user_headimgurl'], $orderPayInfo['_id'], $now, $pointsRuleInfo['points'], $pointsRuleInfo['item_category'], $pointsRuleInfo['item']);
