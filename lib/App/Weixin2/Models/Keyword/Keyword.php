...
class Keyword extends \App\Common\Models\Weixin2\Keyword\Keyword
{
     ...
     * @return array
     */
    public function getKeywordByType($authorizer_appid, $component_appid, $now, $fuzzy)
    {
        $keywordList = array();
        $cacheKey = "keyword:authorizer_appid:{$authorizer_appid}:component_appid:{$component_appid}:fuzzy:{$fuzzy}:n:" . date("YmdH", $now);
        $cacheKey = \App\Common\Utils\Helper::myCacheKey(__CLASS__, $cacheKey);
        $cache = $this->getDI()->get('cache');
        $keywordList = $cache->get($cacheKey);
        if (empty($keywordList)) {
            $rst = $this->getListByFuzzy($authorizer_appid, $component_appid, $now, $fuzzy);
            $keywordList = array();
            if (!empty($rst)) {
                foreach ($rst as $row) {
    ...
    public function matchKeyWord($msg, $authorizer_appid, $component_appid, $now, $fuzzy = false)
    {
        $msg = trim($msg);
        $keywordList = $this->getKeywordByType($authorizer_appid, $component_appid, $now, $fuzzy);
        if (!$fuzzy) {
            $msg = strtolower($msg);
            if (isset($keywordList[$msg])) {
                ...
                return $this->matchKeyWord($msg, $authorizer_appid, $component_appid, $now, true);
            }
        } else {
    ...
    public function getListByFuzzy($authorizer_appid, $component_appid, $now, $fuzzy)
    {
