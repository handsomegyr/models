...
class News extends \App\Common\Models\Qyweixin\LinkedcorpMsg\News
{

    public function getListByLinkedcorpMsgId($linkedcorp_msg_id)
    {
        $ret = $this->findAll(array(
            'linkedcorp_msg_id' => $linkedcorp_msg_id
        ), array('index' => 1, '_id' => -1));
        return $ret;
    }

    public function getArticlesByLinkedcorpMsgId($linkedcorp_msg_id, $msg_type, $isFirst = true)
    {
        $articles = array();
        ...
        if (true || empty($articles)) {
            $rst = $this->getListByLinkedcorpMsgId($linkedcorp_msg_id);
            $articles = array();
            if (!empty($rst)) {
                foreach ($rst as $row) {
                    if ($msg_type == 'news') {
