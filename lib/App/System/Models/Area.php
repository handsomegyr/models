...
class Area extends \App\Common\Models\System\Area
{
    ...
    {
        $key = \App\Common\Utils\Helper::myCacheKey(__CLASS__, __METHOD__, $province);
        $cache = $this->getDI()->get("cache");
        $provinceList = $cache->get($key);
        if (empty($provinceList)) {
            $query = $this->getQuery();
    ...
    {
        $key = \App\Common\Utils\Helper::myCacheKey(__CLASS__, __METHOD__, $city);
        $cache = $this->getDI()->get("cache");
        $provinceList = $cache->get($key);
        if (empty($provinceList)) {
    ...
    {
        $key = \App\Common\Utils\Helper::myCacheKey(__CLASS__, __METHOD__, $level);
        $cache = $this->getDI()->get("cache");
        $provinceList = $cache->get($key);
        if (empty($provinceList)) {
